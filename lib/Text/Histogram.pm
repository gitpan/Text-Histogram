...
Text::Histogram - Make Text Histogram (Upright Bars) Charts
...
Text::Histogram creates graphical charts for terminal displays or any other display device where bitmap graphics is not available or desired!
You can supply the Y axel legend (vertical) values or Text::Histogram can calculate them from the input values.
...
Text::Histogram is a purely object-oriented module.
...
the web interface at L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=Text-Histogram>.  I will be notified, and then you'll
